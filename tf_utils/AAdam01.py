...
class AAdamOptimizer01(optimizer.Optimizer):
    ...
    def __init__(self, learning_rate=0.001, beta1=0.9, beta2=0.999, use_locking=False, epsilon=1e-8, name="AAdam01"):
        super(AAdamOptimizer01, self).__init__(use_locking, name)
    ...
    def _apply_dense(self, grad, var):
        ...
        m_t = tf.where((tf.sign(grad) * tf.sign(p_grad)) < 0, tf.sign(g_t)*(tf.abs(g_t) -  0.1*tf.abs(d) ), tf.sign(g_t)*(tf.abs(g_t) +  0.1*tf.abs(d) ))
        d_t = d.assign(m_t)
